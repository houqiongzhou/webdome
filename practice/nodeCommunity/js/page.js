	...
	page.prototype = {
		...
		*@return {boolean}
		...
		createHtml: function() {
			...
			if(this.totle < 1 || this.nowpage > thid.totle) {return false};

			// 如果当前页数>1的话
			this.nowpage > ? this.htmlStr.push('<a href="#" class="prev">上一页</a>') : this.htmlStr.push('<span class="current prev">上一页</span>');
			...
			// 如果当前页数<总页数的话
			this.nowpage<this.totle ? this.htmlStr.push('<a href="#">下一页</a>') : this.htmlStr.push('<span class="current next">下一页</span>');
			// 设置html的值
		...
		*@return {boolean}
		...
		contains: function(element, oParent) {
			if(oParent.contains) {
				tryutn oParent.contains(element)
			} else if(oParent.compareDocumentPossition) {
		...
		*@return {boolean}
		 */
		ifParent: function(element,tagName) {
		...
		if(typeof dwfis !== "undefined" && dwfis.define === 'function') {
			module.exports = page;
		} else {
